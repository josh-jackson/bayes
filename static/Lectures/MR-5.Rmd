...
Interactions reflect when the influence of one variables depends on another. No longer can speak of main effects, now one needs to describe the association between two (or more) variables and the DV. The result is the interpretation is the hard part, not necessarily the modeling. 
...
```{r}
MR %>% 
data_grid(SS_c = seq_range(SS_c, n = 20), FQ_c = c(-2.5,0,2.5), .model = MR) %>% add_fitted_draws(mr.13)
